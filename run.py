...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--grayscale", action="store_true", help="do not apply colorful palette"
    )
    ...
    video_depth_anything.load_state_dict(
        torch.load(
            f"./checkpoints/video_depth_anything_{args.encoder}.pth", map_location="cpu"
        ),
        strict=True,
    )
    ...
    save_video(
        depths, depth_vis_path, fps=fps, is_depths=True, grayscale=args.grayscale
    )

    if args.save_npz:
        depth_npz_path = os.path.join(
            args.output_dir, os.path.splitext(video_name)[0] + "_depths.npz"
        )
        np.savez_compressed(depth_npz_path, depths=depths)
    if args.save_exr:
        depth_exr_dir = os.path.join(
            args.output_dir, os.path.splitext(video_name)[0] + "_depths_exr"
        )
